...
void adicionar_ao_ranking(Player *pl)
{
    ...
    EntradaRanking entry;
    strncpy(entry.pl_nome, pl->nome, 50);

    if (pl->karma>5)
        entry.final = PACIFISTA;
    else if(pl->karma>0)
        entry.final = NEUTRO;
    else 
        entry.final = GENOCIDA;
  
    entry.num_mortes = 0;
    entry.num_mortes += pl->medidor_lembranca[Cavaleiro_Vazio];
    entry.num_mortes += pl->medidor_lembranca[Cruciator];
    entry.num_mortes += pl->medidor_lembranca[Aphanos];
    entry.num_mortes += pl->medidor_lembranca[Cerberus];
    entry.num_mortes += pl->medidor_lembranca[Iowa];
    entry.num_mortes += pl->medidor_lembranca[Rei_Caido];
...
void visualizar_ranking(WINDOW *tela_ranking)
{
    FILE *fp = fopen("saves/ranking.bin", "rb");
    ...
    mvwprintw(tela_ranking, 5, getmaxx(stdscr)/2 - 25, "█████▄   ▄▄▄  ▄▄  ▄▄ ▄▄ ▄▄ ▄▄ ▄▄  ▄▄  ▄▄▄▄");
    mvwprintw(tela_ranking, 6, getmaxx(stdscr)/2 - 25, "██▄▄██▄ ██▀██ ███▄██ ██▄█▀ ██ ███▄██ ██ ▄▄");
    mvwprintw(tela_ranking, 7, getmaxx(stdscr)/2 - 25, "██   ██ ██▀██ ██ ▀██ ██ ██ ██ ██ ▀██ ▀███▀");
    ...
    wattron(tela_ranking, A_BOLD | COLOR_PAIR(COR_DESTAQUE));
    mvwprintw(tela_ranking, y + 3, 27+(108-5)/2, "Aperte ENTER para voltar ao menu");
    wattroff(tela_ranking, A_BOLD | COLOR_PAIR(COR_DESTAQUE));

    wrefresh(tela_ranking);
    int tecla;
    while (true)
    {
        tecla = wgetch(tela_ranking);
        if (tecla == KEY_ENTER || tecla==10 || tecla=='\n')
        {
